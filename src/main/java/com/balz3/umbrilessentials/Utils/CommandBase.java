...
import com.balz3.umbrilessentials.Main;
...
public abstract class CommandBase extends BukkitCommand implements CommandExecutor {
    ...
    public void sendUsage(CommandSender sender){
        MessageManager.send(sender, getUsage());
    }
    ...
    public boolean execute(CommandSender sender, String alias, String[] args) {
        ...
        if(playerOnly && !(sender instanceof Player)){
            MessageManager.send(sender, "&cOnly players can use this command.");
        ...
        if(permission != null && !sender.hasPermission(permission)){
            MessageManager.send(sender, "&cYou do not have permission to use this command.");
        ...
        if(delayedPlayers != null && sender instanceof Player){
            ...
            if(delayedPlayers.contains(player.getName())){
                MessageManager.send(sender, "&cPlease wait before using this command again.");
